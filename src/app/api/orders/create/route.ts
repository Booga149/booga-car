...
export async function POST(req: NextRequest) {
  try {
    ...
    for (const item of items) {
      const staticProduct = ENGINEER_SYSTEMS_PRODUCTS.find(p => p.id === (item.product_id || item.id));
      const dbProduct = staticProduct ? { ...staticProduct, stock_quantity: 999, seller_id: null } : products.find((p: any) => p.id === (item.product_id || item.id));
